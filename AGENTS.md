...
- Cada unidad visual reusable usa archivos reales separados `.html`, `.css`, `.ts` en su carpeta, más `index.ts` cuando corresponda. No usar `.template.ts`, `.styles.ts`, Sass/SCSS ni generar el markup/estilo principal desde TypeScript.
- `src/tokens/tokens.json` es la fuente canónica de design tokens; `src/styles/tokens.css` es su bridge 1:1 mediante CSS custom properties.
...
- W1: `AGENTS.md`, `docs/**` y contratos/arquitectura asignados explícitamente.
...
- Integración: branch/worker dedicado; sólo composition root, adapters de integración, migraciones estructurales acordadas y conflictos explícitamente autorizados.
